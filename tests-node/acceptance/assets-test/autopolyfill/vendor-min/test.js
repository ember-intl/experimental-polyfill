...
const debug = require('debug')('assets-test:autopolyfill:vendor-min');

const { describe, before, after, it } = mocha;

describe('autopolyfill/vendor-min acceptance', function() {
  this.timeout(400000);
  ...
  before(function() {
    ...
    return app
      .create('fixture', {
        fixturesPath: 'tests-node/acceptance/assets-test/autopolyfill/vendor-min/'
      })
  ...
  it('assets/vendor.js should contain min intl polyfill', function() {
    ...
    return request(`http://localhost:49741/assets/vendor.js`).then(res => {
      ...
      expect(res.body.includes(data.fr_fr)).to.equal(false);
